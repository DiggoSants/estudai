...
*{box-sizing:border-box;margin:0;padding:0}body{background:var(--bg);color:var(--text);font-family:'Montserrat',sans-serif;line-height:1.6;min-height:100vh}h1,h2,h3,.logo-text,.value,.nav-section,.panel-title{font-family:'Montserrat',sans-serif}a{text-decoration:none;color:inherit}.atmo{position:fixed;inset:0;pointer-events:none;z-index:0;background:radial-gradient(ellipse 900px 600px at -10% 10%,rgba(0,229,160,.055),transparent 65%),radial-gradient(ellipse 800px 560px at 110% 80%,rgba(0,184,255,.055),transparent 65%)}
.topbar{position:sticky;top:0;z-index:5;display:flex;align-items:center;justify-content:space-between;gap:14px;padding:14px 24px;background:rgba(6,11,24,.86);border-bottom:1px solid var(--border);backdrop-filter:blur(18px)}.menu-btn{width:44px;height:44px;border:1px solid var(--border);border-radius:12px;background:rgba(255,255,255,.04);display:grid;place-items:center;gap:4px;cursor:pointer}.menu-btn span{display:block;width:19px;height:2px;border-radius:2px;background:var(--text)}.logo{display:flex;align-items:center;gap:10px;min-width:0}.logo-icon{width:40px;height:40px;border-radius:11px;background:var(--grad);display:grid;place-items:center;font-weight:800;color:#060b18;flex:0 0 auto}.logo-text{font-size:22px;font-weight:800;background:var(--grad);-webkit-background-clip:text;-webkit-text-fill-color:transparent}.drawer-overlay{position:fixed;inset:0;z-index:8;background:rgba(0,0,0,.45);opacity:0;pointer-events:none;transition:.2s}.sidebar{position:fixed;top:0;left:0;z-index:9;width:min(310px,88vw);height:100vh;background:rgba(8,14,28,.98);border-right:1px solid var(--border);padding:24px 20px;display:flex;flex-direction:column;transform:translateX(-104%);transition:transform .24s ease;box-shadow:40px 0 100px rgba(0,0,0,.35)}body.menu-open .sidebar{transform:translateX(0)}body.menu-open .drawer-overlay{opacity:1;pointer-events:auto}.drawer-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:26px}.drawer-close{width:40px;height:40px;border:1px solid var(--border);border-radius:12px;background:rgba(255,255,255,.04);color:var(--text);font-size:22px;cursor:pointer}.nav-section{color:var(--muted);font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:0;margin:24px 0 10px}.nav-link{display:flex;gap:10px;align-items:center;color:var(--muted);font-weight:700;padding:11px 12px;border-radius:12px;margin-bottom:4px}.nav-link:hover,.nav-link.active{background:rgba(0,229,160,.08);color:var(--text)}.bottom{margin-top:auto;border-top:1px solid var(--border);padding-top:18px}.userbox{display:flex;gap:10px;align-items:center;background:rgba(255,255,255,.035);border:1px solid var(--border);border-radius:14px;padding:10px}.avatar{width:38px;height:38px;border-radius:12px;display:grid;place-items:center;font-weight:800}.userbox strong{display:block;font-size:14px}.userbox span{color:var(--muted);font-size:12px}.logout{display:block;text-align:center;color:var(--muted);border:1px solid var(--border);border-radius:12px;padding:10px;margin-top:10px;font-weight:800}.logout:hover{border-color:var(--danger);color:var(--danger)}
.main{position:relative;z-index:1;padding:38px;max-width:1280px;margin:0 auto}.header{display:flex;justify-content:space-between;gap:20px;align-items:flex-start;margin-bottom:28px}.header h1{font-size:42px;line-height:1.08;letter-spacing:0;margin-top:6px}.btn{display:inline-flex;align-items:center;justify-content:center;border-radius:12px;padding:14px 22px;background:var(--grad);color:#060b18;font-family:'Montserrat';font-weight:800;box-shadow:0 0 34px rgba(0,229,160,.24);text-align:center}
...
<aside class="sidebar">
  <div class="drawer-head">
    <a href="<?= url('dashboard') ?>" class="logo"><div class="logo-icon">E</div><span class="logo-text">Estudai</span></a>
    <button class="drawer-close" type="button" aria-label="Fechar menu" onclick="document.body.classList.remove('menu-open')">×</button>
  </div>
...
<main class="main">
  <header class="header">
    <div><h1><?= e($saudacao) ?>, <?= e($primeiroNome) ?>.</h1><p>Seu mapa de desempenho está pronto para guiar o próximo estudo.</p></div>
  </header>

  <section class="level">
    <div class="level-top">
      <div class="level-badge" style="border-color:<?= e($nivel['cor']) ?>;background:<?= e($nivel['cor']) ?>18"><?= e($nivel['icone']) ?></div>
      <div><h2 style="color:<?= e($nivel['cor']) ?>"><?= e($nivel['nome']) ?></h2><span>Nível <?= (int) $nivel['nivel'] ?> - <?= number_format((int) $user['xp']) ?> XP total</span></div>
      <div style="margin-left:auto"><h2><?= (int) $nivel['progresso'] ?>%</h2><span>para o próximo nível</span></div>
    </div>
  ...
  <?php $totalSimulados = (int) ($stats['total'] ?? 0); $totalAcertos = (int) ($stats['acertos'] ?? 0); $totalQuestoes = (int) ($stats['questoes'] ?? 0); $media = isset($stats['media']) && $stats['media'] !== null ? (float) $stats['media'] : 0.0; ?>
  <section class="grid stats">
    ...
    <div class="card"><small>Média geral</small><span class="value"><?= $media > 0 ? round($media) . '%' : '-' ?></span><?php if ($media >= 70): ?><span class="delta">ótimo ritmo</span><?php elseif ($media > 0): ?><span class="delta bad">pode melhorar</span><?php endif; ?></div>
    <div class="card"><small>Dias seguidos</small><span class="value"><?= (int) $user['streak'] ?></span><?php if ((int) $user['streak'] >= 3): ?><span class="delta">sequência ativa</span><?php endif; ?></div>
  </section>
  ...
  <section class="panel ai">
    ...
    <div style="margin-top:18px"><a class="btn" href="<?= url('simulado') ?>">Gerar novo diagnóstico</a></div>
  </section>

  <section class="grid two">
    <div class="panel">
      <div class="panel-title">Desempenho por matéria <span>últimos 30 dias</span></div>
